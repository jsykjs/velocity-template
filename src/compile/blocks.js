  ...
  Object.assign(Velocity.prototype, {
    ...
    getMacro: function(ast, bodyContent) {
      ...
      if (!macro) {
        ...
        var guid = Symbol("guid").toString();
        var contextId = 'macro:' + ast.id + ':' + guid;

        args.forEach(function(ref, i) {
          if (callArgs[i]) {
    ...
    eval: function(str, local, contextId) {
      ...
      if (!local) {
        ...
        contextId = contextId || ('eval:' + Symbol('guid').toString());
    ...
    getBlockEach: function(block) {
      ...
      if (Array.isArray(_from)) {
        ...
        var len = Object.keys(_from).length;
        Object.keys(_from).forEach(function(key, i) {
          if (this._state.break) {
            return;
          }
          local[_to] = _from[key];
